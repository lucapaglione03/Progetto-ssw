...
export class Archivio {
  ...
  aggiornaInventario(json: string): void {
    //costruisce l'array dalla stringa JSON
    JSON.parse(json).forEach((item: any) => {
      //parsing per avere l'array
      const libro = new Volume(
        item.autore,
        item.titolo,
        item.posizione,
        item.nominativo
  ...
  trovaLibri(regex: RegExp): any {
    const risultatiRicerca = this.inventario.filter((libro) =>
      ricerca(libro, regex)
    );

    function ricerca(libro: Volume, regex: RegExp): boolean {
      let stringaRicerca: string = libro.titolo
        .concat(' ', libro.autore, ' ', libro.posizione)
        .toLowerCase();
